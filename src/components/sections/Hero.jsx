...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      <div className="container-luxe relative z-10">
        ...
        >
          <div className="mx-auto flex max-w-3xl flex-col items-center text-center lg:mx-0 lg:items-start lg:text-left">
            ...
            >
              <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/[0.04] px-4 py-2 text-ivory/80">
          ...
          >
            <div className="absolute -inset-4 rounded-[2rem] bg-gold/15 blur-3xl animate-pulse" />
            <div className="relative overflow-hidden rounded-[1.75rem] border border-gold/40 bg-cosmic-950/80 shadow-glow-lg">
              {heroVideoUrl ? (
                ...
                <div className="relative aspect-[3/4] w-full overflow-hidden">
                  ...
                  />
                  <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-cosmic-950/50 via-transparent to-transparent" />
                ...
                >
                  ...
                  {/* Gradient overlays */}
                  <div className="absolute inset-0 bg-gradient-to-t from-cosmic-950/90 via-cosmic-950/20 to-cosmic-950/60 transition-opacity duration-300 group-hover:opacity-80" />

                  {/* Top badges */}
                  <div className="absolute top-4 inset-x-4 flex items-center justify-between z-10">
                    <span className="glass-gold inline-flex items-center gap-2 rounded-full px-3.5 py-1 text-[11px] font-semibold uppercase tracking-wider text-gold">
                      <span className="h-2 w-2 rounded-full bg-gold animate-ping" />
                      {site.name}
                    </span>
                    <span className="rounded-full border border-white/20 bg-cosmic-950/70 px-3 py-1 text-[10px] font-medium tracking-wider text-ivory/80 backdrop-blur-md">
                      Video Placeholder
                    </span>
                  </div>
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center z-10">
                    <div className="relative flex items-center justify-center">
                      <span className="absolute h-24 w-24 rounded-full bg-gold/20 animate-ping opacity-75" />
                      <span className="absolute h-20 w-20 rounded-full bg-gold/30 blur-sm" />
                      <button
                        type="button"
                        aria-label="Play introduction video placeholder"
                        className="relative flex h-16 w-16 items-center justify-center rounded-full border-2 border-gold bg-gold-gradient text-cosmic-950 shadow-[0_0_30px_rgba(218,178,92,0.6)] transition-transform duration-300 group-hover:scale-110"
                      >
                        <Play className="h-7 w-7 fill-current ml-1" />
                      </button>
                    </div>
                    <p className="mt-4 font-display text-sm font-semibold tracking-wider text-ivory drop-shadow-md">
                      Watch Introduction
                    </p>
                  </div>

                  {/* Bottom info bar */}
                  <div className="absolute bottom-4 inset-x-4 z-10 rounded-xl border border-white/10 bg-cosmic-900/80 p-3.5 backdrop-blur-md">
                    <div className="flex items-center justify-between text-xs">
                      <div>
                        <p className="font-medium text-ivory">Sacred Introduction Preview</p>
                        <p className="text-[10px] text-ivory/50">Vedic Astrology &amp; Divine Solutions</p>
                      </div>
                      <span className="rounded-md border border-gold/30 bg-gold/10 px-2 py-0.5 text-[10px] text-gold font-medium">
                        HD 1080p
                      </span>
                    </div>
                  </div>
      ...
      {showVideoModal && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-cosmic-950/80 p-4 backdrop-blur-md"
          onClick={() => setShowVideoModal(false)}
        >
          <div
            className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-gold/40 bg-cosmic-900 p-6 shadow-2xl text-center"
          ...
          >
            ...
            <p className="mt-3 text-sm text-ivory/70 leading-relaxed">
              ...
              <code className="rounded bg-black/40 px-1.5 py-0.5 text-gold font-mono text-xs">
                heroVideoUrl
              </code>{' '}
              in <span className="text-gold">src/data/site.js</span>.
            </p>
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-center">
              <Button href="#contact" variant="gold" size="sm" onClick={() => setShowVideoModal(false)}>
