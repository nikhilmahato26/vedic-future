...
export default function MatchingPage({ service }) {
    ...
    <>
      ...
      <section className="container-luxe relative z-10 pb-24">
        {ready
          ...
          : <MatchForm onSubmit={(b, g, sys) => { if(service) setPendingAuth({b, g, sys}); else { setSearchParams({ ...birthToSearch(b, 'm'), ...birthToSearch(g, 'f'), sys }); window.scrollTo({ top: 0, behavior: 'smooth' }); } }} />}
      ...
      {service && (
        <CheckoutModal
          ...
          isUnlockMode={true}
          onSuccess={() => {
            setSearchParams({ ...birthToSearch(pendingAuth.b, 'm'), ...birthToSearch(pendingAuth.g, 'f'), sys: pendingAuth.sys });
